...
print("🔧 Corrigiendo sidepanel.js...")

# Leer archivo
with open(r'd:\Trabajador\aviator-tracker-extension\sidepanel.js', 'r', encoding='utf-8') as f:
    content = f.read()

# Método completo a insertar después de setupEventListeners()
methods_to_insert = '''
...
# Buscar el final de setupEventListeners() e insertar
pattern = r'(    setupEventListeners\(\) \{[^}]+\})\s*\n'
match = re.search(pattern, content, re.DOTALL)

if match:
    insert_pos = match.end()
    content = content[:insert_pos] + methods_to_insert + '\n' + content[insert_pos:]
    print("✅ Métodos de estrategias insertados")
else:
